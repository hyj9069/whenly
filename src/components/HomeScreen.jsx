...
// ── 방 아이템 (공통) ─────────────────────────────
function RoomItem({ room, onEnterRoom }) {
  const [yr, mo] = room.month.split('-')
  return (
    <button onClick={() => onEnterRoom(room)}
      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 15px', background: '#fff', border: '2px solid rgba(0,0,0,.07)', borderRadius: 16, cursor: 'pointer', textAlign: 'left', boxShadow: '0 2px 8px var(--shadow)', fontFamily: 'inherit', width: '100%' }}>
...
// ── 모임 탭 ─────────────────────────────────────
function RoomsTab({ myRooms, onCreate, onJoinCode, onEnterRoom }) {
  return (
    <>
      {myRooms.length > 0 ? (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {myRooms.map(r => <RoomItem key={r.id} room={r} onEnterRoom={onEnterRoom} />)}
          </div>
        </div>
      ) : (
...
export default function HomeScreen({ user, myName, myRooms, initialTab = 'home', onCreate, onJoinCode, onEnterRoom, onLogout, onUpdateName }) {
  ...
  return (
    <div className="screen" style={{ paddingTop: 24, paddingBottom: 86 }}>
      ...
      {tab === 'rooms'   && <RoomsTab   myRooms={myRooms} onCreate={onCreate} onJoinCode={onJoinCode} onEnterRoom={onEnterRoom} />}
